...
const ToDoPage = () => {
    ...
    const fetchCategories = async () => {
      try {
        const responseData = await sendRequest(
          `${process.env.REACT_APP_BACKEND_API_URL}/todo/categories/${auth.UID}`,
          "GET",
          null,
  ...
  useEffect(() => {
    //when submitting the 2doitem modal, it reloads modal if loadedTasks changes but if setTaskModalCHanges first it doesnt update loadedTasks
    setTaskModal(false);
    setEditTask(false);
  }, [loadedTasks]);
  ...
  useEffect(() => {
    ...
    const fetchTasks = async () => {
      try {
        const responseData = await sendRequest(
          `${process.env.REACT_APP_BACKEND_API_URL}/todo/getItems/${auth.UID}/${loadedCategory.name}`,
          "GET",
          null,
          { Authorization: `Bearer ${auth.token}` }
        );
  ...
  const toggleRight = () => {
    const currentCatIndex = loadedCategories.findIndex(
      (cat) => cat.name === loadedCategory.name
    );
  ...
  return (
    <React.Fragment>
      ...
      {!isLoading && loadedCategory && loadedTasks && (
        <ToDoItemModal
          category={loadedCategory}
          open={taskModal}
          taskId={editTask}
          submitted={(task) => {
            editTask ? submitEditHandler(task) : submitNewHandler(task);
          }}
          onError={handleTaskModalError}
          onClear={() => {
            setTaskModal(false);
            setEditTask();
          }}
      ...
      <SwipeableHook
        className="todo-page__contents"
        onSwipedLeft={toggleRight}
        onSwipedRight={toggleLeft}
      >
        {/*This is a div but swipeable events*/}
        {isLoading && (
        ...
        {!isLoading && loadedCategories && (
          <CategoryList
            onChangeCategory={changeLoadedCategoryHandler}
        ...
        {!isLoading && loadedCategory && (
          <div className="todo-page__header">
            ...
            <div className="todo-page__category-header">
              <Button
                className="todo-page__arrow-item-button"
                onClick={toggleLeft}
              >
                &lt;
              </Button>
        ...
        {!isLoading && categoryEditor && loadedCategory && (
          <CategoryEditor
